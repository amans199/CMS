...
function InvoicesItems() {
  ...
  useEffect(() => {
    fetchAllInvoicesItems();
  }, []);

  const fetchAllInvoicesItems = async () => {
    try {
      const response = await axios.get("/api/invoices/items");
      setInvoicesItems(response?.data || []);
    } catch (error) {
      console.error("Fetching InvoicesItems failed:", error);
    }
  };

  const handleDeleting = async (name, id) => {
    try {
      const response = await axios.delete(`/api/invoices/items/${id}`);
  ...
  const handleAdding = async () => {
    if (!name) return;
    try {
      const response = await axios.post(`/api/invoices/items`, { name });
  ...
  const rows = items.map((item) => ({
    id: <>{item.id}</>,
    name: <SoftTypography>{item.name}</SoftTypography>,
    action: (
      <>
        <SoftButton variant="primary" onClick={() => handleDeleting(item.name, item.id)}>
          Delete
        </SoftButton>
      </>
    ),
        ...
        <SoftBox mb={3}>
          <Card>
            <SoftBox display="flex" justifyContent="space-between" alignItems="center" p={3}>
              <SoftTypography variant="h6">InvoicesItems table</SoftTypography>
